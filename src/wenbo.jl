...
function _transform1!(out::AbstractVector, f::AbstractVector, l)
	pointerA = 1
    while pointerA <= l
        while pointerA <= l && @inbounds f[pointerA] >= 0.5
            out[pointerA] = 0f0
            pointerA+=1
        end 
        pointerB = pointerA
        while pointerB <= l && @inbounds f[pointerB] < 0.5
            pointerB+=1
        end
        if pointerB > l
            pointerA != 1 || return _fill_with_inf(out)
            i = pointerA
            temp=i-1
            while i<=l
                @inbounds out[i]=(i-temp)^2
                i+=1
            end
        else
            if pointerA == 1
...
function _transform2!(out::AbstractVector, f::AbstractVector, l, d_left, d_right)
    # 1.    1st pass, left -> right
    # 1.1   mid section
	head_idx = 0
    i = d_left
    @inbounds a = f[i]
    x_prev = 0
    while (i+=1) <= d_right
		@inbounds i > head_idx+1 && (out[i-1] = f[i-1])
        @inbounds b = f[i]
        x = min(l-i+1, ceil(Int, (b-a-1)/2))
        step, x_prev = max(1,x_prev), x
		step <= x || (a=b;continue)
        value, a = muladd(step, step, a), b
		diff = muladd(step, 2f0, 1f0)
		target_idx = step+i-1
        while step <= x
			@inbounds out[target_idx] = target_idx > d_right ? value : min(f[target_idx], value)
            head_idx = max(head_idx, target_idx)
            value += diff
            diff += 2f0
			target_idx += 1
            step += 1
	...
	x = l-i+1
	step = max(1, x_prev)
	diff, value = muladd(step, 2f0, 1f0), muladd(step, step, a)
	target_idx = step+i-1
	while step <= x
		@inbounds out[target_idx] = target_idx > head_idx ? value : min(out[target_idx], value)
		value += diff
		diff += 2f0
		target_idx += 1
		step += 1
	end
    # 2.    2nd pass, right -> left
	head_idx = l+1
    # 2.1   mid section
	i = d_right
    @inbounds a = f[i]
    ...
    while (i-=1) >= d_left
		...
		target_idx = i-step+1
        while step <= x
			@inbounds out[target_idx] = target_idx < d_left ? value : min(out[target_idx], value)
			head_idx = min(head_idx, target_idx)
            value += diff
            diff += 2f0
			target_idx -= 1
            step += 1
...
function transform(f::AbstractMatrix, tfm::Wenbo)
	l, l2 = size(f)
	out, out2 = similar(f), similar(f)
	# phase 1
	flags = Array{Bool}(undef, l)
	for i = 1 : l
		@inbounds flags[i] = _transform1!(@view(out[i, :]), @view(f[i, :]), l2)
	end
	...
	# phase 2
	for i = 1 : l2
		@inbounds _transform2!(@view(out2[:,i]), @view(out[:,i]), l, d1, d2)
	end
	return out2
end
...
function transform(f::AbstractArray, tfm::Wenbo)
	...
	for i in CartesianIndices(@view(out[1,:,:]))
		@inbounds flags[i] = _transform1!(@view(out[:, i]), @view(f[:, i]), l)
	end
	...
	# phase 2
	for i in CartesianIndices(@view(out[:,1,d1:d2]))
		@inbounds _transform2!(@view(out2[i[1], :, i[2]+d1-1]), @view(out[i[1], :, i[2]+d1-1]), l2, d3, d4)
	end
	# phase 3
	for i in CartesianIndices(@view(out[:,:,1])) 
		@inbounds _transform2!(@view(out[i, :]), @view(out2[i, :]), l3, d1, d2)
	end
	return out
end 
...
function transform(f::AbstractArray, tfm::Wenbo, nthreads::Number)
	...
	# phase 1
	flags = Array{Bool, 2}(undef, l2, l3)
	Threads.@threads for i in CartesianIndices(@view(out[1,:,:]))
		@inbounds flags[i] = _transform1!(@view(out[:, i]), @view(f[:, i]), l)
	end
	d1, d2, d3, d4 = 0,0,0,0
	@sync begin
    ...
    d1==d2==0 && return out
	# phase 2
	Threads.@threads for i in CartesianIndices(@view(out[:,1,d1:d2]))
		@inbounds _transform2!(@view(out2[i[1], :, i[2]+d1-1]), @view(out[i[1], :, i[2]+d1-1]), l2, d3, d4)
	end
	# phase 3
	Threads.@threads for i in CartesianIndices(@view(out[:,:,1])) 
...
function _kernel_2D_1_1!(out, f, row_l, l)
	...
	row = cld(i, row_l)
	col = i%row_l+1
	@inbounds f[row, col] < 0.5f0 || return
	ct = 1
	curr_l = min(col-1, row_l-col)
	while ct <= curr_l
		@inbounds (f[row, col-ct] < 0.5f0 && f[row, col+ct] < 0.5f0) || (out[row, col] = ct*ct; return)
		ct += 1
	end
	while ct < col
		@inbounds f[row, col-ct] < 0.5f0 || (out[row, col] = ct*ct; return)
		ct += 1    
	end
	while col+ct <= row_l
		@inbounds f[row, col+ct] < 0.5f0 || (out[row, col] = ct*ct; return)
...
# ╔═╡ 927f25f9-1687-415f-b5bb-8a8f40afdd0f
 function _kernel_2D_2!(org, out, row_l, col_l, l)
    ...
    @inbounds while ct < curr_l && row+ct <= col_l
		...
		@inbounds temp >= out[row, col] || (out[row, col] = temp; curr_l = CUDA.sqrt(temp))
        ct += 1
    end
    ct = 1
...
function _kernel_2D_1_1_batch!(out, f, row_l, l, channel_idx, batch_idx)
	...
	row = cld(i, row_l)
	col = i%row_l+1
	@inbounds f[row, col, channel_idx, batch_idx] < 0.5f0 || return
	ct = 1
	curr_l = min(col-1, row_l-col)
	while ct <= curr_l
		@inbounds (f[row, col-ct, channel_idx, batch_idx] < 0.5f0 && f[row, col+ct, channel_idx, batch_idx] < 0.5f0) || (out[row, col, channel_idx, batch_idx] = ct*ct; return)
		ct += 1
	end
	while ct < col
		@inbounds f[row, col-ct, channel_idx, batch_idx] < 0.5f0 || (out[row, col, channel_idx, batch_idx] = ct*ct; return)
		ct += 1    
	end
	while col+ct <= row_l
		@inbounds f[row, col+ct, channel_idx, batch_idx] < 0.5f0 || (out[row, col, channel_idx, batch_idx] = ct*ct; return)
...
# ╔═╡ aaf5a46a-67c7-457b-bc83-d1c2163583b8
 function _kernel_2D_2_batch!(org, out, row_l, col_l, l, channel_idx, batch_idx)
    ...
    @inbounds while ct < curr_l && row+ct <= col_l
        ...
        @inbounds temp >= out[row, col, channel_idx, batch_idx] || (out[row, col, channel_idx, batch_idx] = temp; curr_l = CUDA.sqrt(temp))
        ct += 1
    end
    ct = 1
    @inbounds while ct < curr_l && row > ct
...
function _kernel_3D_1_1!(out, f, dim2_l, dim3_l, l)
    ...
    dim3 = temp2 % dim3_l + 1
    # 1d DT along dim2
    @inbounds f[dim1, dim2, dim3] < 0.5f0 || return
	ct = 1
	curr_l = CUDA.min(dim2-1, dim2_l-dim2)
	while ct <= curr_l
		@inbounds (f[dim1, dim2-ct, dim3]<0.5f0 && f[dim1, dim2+ct, dim3]<0.5f0) || (out[dim1, dim2, dim3] = ct*ct; return)
		ct += 1
	end
	while ct < dim2
		@inbounds f[dim1, dim2-ct, dim3]<0.5f0 || (out[dim1, dim2, dim3] = ct*ct; return)
		ct += 1    
	end
...
function _kernel_3D_2!(out, org, dim1_l, dim2_l, dim3_l, l)
    ...
    @inbounds while ct < curr_l && dim1+ct <= dim1_l
        @inbounds temp = muladd(ct,ct,org[dim1+ct, dim2, dim3])
		@inbounds temp >= out[dim1, dim2, dim3] || (out[dim1, dim2, dim3] = temp; curr_l = CUDA.sqrt(temp))
        ct += 1
    end
    ct = 1
...
function _kernel_3D_3!(out, org, dim2_l, dim3_l, l)
    ...
    @inbounds while ct < curr_l && dim3+ct <= dim3_l
        @inbounds temp = muladd(ct,ct,org[dim1, dim2, dim3+ct])
		@inbounds temp >= out[dim1, dim2, dim3] || (out[dim1, dim2, dim3] = temp; curr_l = CUDA.sqrt(temp))
        ct += 1
    end
    ct = 1
...
function _kernel_3D_1_1_batch!(out, f, dim2_l, dim3_l, l, channel_idx, batch_idx)
    ...
    dim3 = temp2 % dim3_l + 1
    # 1d DT alone dim2
	@inbounds f[dim1, dim2, dim3, channel_idx, batch_idx] < 0.5f0 || return
	ct = 1
	curr_l = CUDA.min(dim2-1, dim2_l-dim2)
	while ct <= curr_l
		@inbounds (f[dim1, dim2-ct, dim3, channel_idx, batch_idx] < 0.5f0 && f[dim1, dim2+ct, dim3, channel_idx, batch_idx] < 0.5f0) || (out[dim1, dim2, dim3, channel_idx, batch_idx] = ct*ct; return)
		ct += 1
	end
	while ct < dim2
		@inbounds f[dim1, dim2-ct, dim3, channel_idx, batch_idx] < 0.5f0 || (out[dim1, dim2, dim3, channel_idx, batch_idx] = ct*ct; return)
		ct += 1    
	end
	while dim2+ct <= dim2_l
		@inbounds f[dim1, dim2+ct, dim3, channel_idx, batch_idx] < 0.5f0 || (out[dim1, dim2, dim3, channel_idx, batch_idx] = ct*ct; return)
...
function _kernel_3D_2_batch!(out, org, dim1_l, dim2_l, dim3_l, l, channel_idx, batch_idx)
    ...
    @inbounds while ct < curr_l && dim1+ct <= dim1_l
        @inbounds temp = muladd(ct,ct,org[dim1+ct, dim2, dim3, channel_idx, batch_idx])
		@inbounds temp >= out[dim1, dim2, dim3, channel_idx, batch_idx] || (out[dim1, dim2, dim3, channel_idx, batch_idx] = temp; curr_l = CUDA.sqrt(temp))
        ct += 1
    end
    ct = 1
    @inbounds while ct < curr_l && dim1-ct > 0
...
function _kernel_3D_3_batch!(out, org, dim2_l, dim3_l, l, channel_idx, batch_idx)
    ...
    @inbounds curr_l = CUDA.sqrt(out[dim1, dim2, dim3, channel_idx, batch_idx])
    @inbounds while ct < curr_l && dim3+ct <= dim3_l
		@inbounds temp = muladd(ct,ct,org[dim1, dim2, dim3+ct, channel_idx, batch_idx])
		@inbounds temp >= out[dim1, dim2, dim3, channel_idx, batch_idx] || (out[dim1, dim2, dim3, channel_idx, batch_idx] = temp; curr_l = CUDA.sqrt(temp))
        ct += 1
    end
    ct = 1
...
function transform(f_org::AbstractArray, tfm::Wenbo, ex)
	...
	# phase 1
	flags = Array{Bool, 2}(undef, l2, l3)
	@floop for i in CartesianIndices(@view(out[1,:,:]))
		@inbounds flags[i] = _transform1!(@view(out[:, i]), @view(f[:, i]), l)
	end
	d1, d2, d3, d4 = 0,0,0,0
	@sync begin
    ...
    d1==d2==0 && return out
	# phase 2
	@floop for i in CartesianIndices(@view(out[:,1,d1:d2]))
		@inbounds _transform2!(@view(out2[i[1], :, i[2]+d1-1]), @view(out[i[1], :, i[2]+d1-1]), l2, d3, d4)
	end
	# phase 3
	@floop for i in CartesianIndices(@view(out[:,:,1])) 
